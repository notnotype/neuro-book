...
**③ Lab fixture**：**无**（`fixtures/index.ts` 42 条 `component` 无一命中 workspace/ToolPanel；`fixtures/*.vue` 36 个无对应文件）。结构性原因见第 1 章跨行事实。注意 `WorkspaceFilePanel` 若将来加文档，其 localStorage 写入（`WorkspaceFilePanel.vue:598/619`）会按能力标签被判 `mountable:false`（`app/component-lab/component-index.ts:88-101`）。
  **2026-09-16 落地**：同名文档已补（`app/components/novel-ide/workspace/WorkspaceFilePanel.md`，标签 `state:local` / `state:shared-read` / `state:shared-write` / `io:read` / `io:mutate` / `persist:local` / `env:clipboard`），Lab 索引已收录并显示「不能在 Lab 里验证（会真的读写产品数据…）」；判定仍为 `mountable:false`，但原因已随 t55 迁移更新（组件内不再有裸键读写，`localStorage` 只在展开记录的旧键一次性迁移路径上，阻断标签里 `state:shared-write` / `io:` 才是主因）。见 [t59 实施记录](tasks/t59-file-panel-lab-coverage/walkthroughs/implementation.md)。
