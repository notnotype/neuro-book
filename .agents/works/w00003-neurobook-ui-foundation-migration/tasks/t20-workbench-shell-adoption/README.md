...
> **状态：已有实现与部分验收，整体合同未闭合。** 下文保留阶段 1 原始范围及按日期写入的历史记录。
> 2026-09-16 后续 Storage、浏览器标题栏和视图迁移以 [当前实施计划](../../storage-implementation-plan.md) 及其 Spec 为准；
> 历史记录中的主题宿主、尺寸 user 归属与分支 sash 限制不能作为新增能力的当前合同。
