/**
 * 被审 revision 的 `app/stores/novel-ide.ts` 逐字节副本（只读夹具）。
 *
 * 提取命令：git show 8263726e:packages/neuro-book/app/stores/novel-ide.ts
 * SHA256 见 review.md「被审文件 SHA256」。
 *
 * 为什么用副本：探针要验证的正是本文件在 8263726e 里的 persist 条目
 * （`storage: legacyBucketStorage()` / `serializer: legacyBucketSerializer`）；
 * 产品文件在审查期间由并行任务（t48）继续编辑，直接 import 会随对方半成品状态漂移。
 * import 路径全部是 `nbook/` 别名，因此副本不需要改一行代码。
 */
import type {
