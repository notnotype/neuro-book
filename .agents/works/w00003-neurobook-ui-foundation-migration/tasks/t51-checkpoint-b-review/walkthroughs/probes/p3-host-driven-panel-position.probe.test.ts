...
function panel(): HTMLElement | null {
    return document.querySelector<HTMLElement>('[data-titlebar-menu-panel="group"]');
}
...
function dropdownRule(): string {
    ...
    console.log(`[P3/CSS] 读取组件源（cwd=${process.cwd()}）：${path}`);
...
describe("P3 宿主驱动展开态下的面板定位", () => {
    ...
    it("宿主驱动路径（Lab 场景 openMenu 预置）：面板没有任何定位，成为 body 下的静态块", async () => {
        ...
        const style = opened!.getAttribute("style") ?? "";
        const rule = dropdownRule();
        console.log(`[P3/host] openMenu=${String(openMenu.value)}；面板内联 style=${JSON.stringify(style)}（空 = 没有任何定位）`);
