...
- 新顺序为：本 Task（B）→ 删除 preview 页面并先保留其场景数据 → Lab 原型（D）→ 种子组件走通全流程 → 回头补写 Lab 界面合同 → 主题切换（C）→ 渐进迁移剩余组件。主题切换与 Lab 没有硬依赖，已由开发者移到走通流程之后；原 P（清退 preview）提前执行，t01 冻结的 31 个场景基线随之降级为历史记录，不再是必须逐条消化的清单。**此后清退 preview 又被顺延到 Lab 之后，当前顺序见 [t05](../t05-component-lab/README.md)。**
...
- **组件目录化：已定为不目录化。**Nuxt 探针证明 `Foo/index.vue` 与 `Foo.vue` 注册名完全相同，目录化技术上无副作用；开发者据此拍板仍不采用，组件保持 `Foo.vue` 与同目录 `Foo.md` 并列的扁平形态，不再重议。
- **fixture 位置：已定。**跟随上一条，集中放在开发专用目录并按整目录排除，不放进组件目录。
