...
新增受控视图 `AgentProfileSettingsView`（`app/components/novel-ide/settings/sections/agent-profile/`），在 Component Lab 中提供完整可操作的 Profile 设置页：默认设置页 + Profile 导航与详情，常用设置优先（使用模型/推理强度/专属设置常驻，高级模型参数与运行策略折叠，诊断与维护独立段）。
