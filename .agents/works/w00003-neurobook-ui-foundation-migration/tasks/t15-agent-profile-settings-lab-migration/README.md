...
- 交付物：`app/components/novel-ide/settings/views/AgentProfileSettingsView.vue` + `.types.ts` + 同名 `.md`；原地重设计 `AgentProfileModelFields.vue`、`ProfileRuntimeSettingsFields.vue`、`AgentProfileDetailPanel.vue`、`AgentProfileDefaultsPanel.vue`；新增 `app/component-lab/fixtures/AgentProfileSettingsViewFixture.vue` 并在 `fixtures/index.ts` 登记 10 个场景（增加 `dialog-window` 内嵌预览）。
...
1. 聚焦测试：`bun run --cwd packages/neuro-book test -- app/components/novel-ide/settings/views/agent-profile/AgentProfileNavList.test.ts app/components/novel-ide/settings/views/agent-profile/profile-runtime-settings.test.ts app/components/novel-ide/settings/views/agent-profile/AgentProfileModelFields.test.ts app/components/novel-ide/settings/views/agent-profile/AgentProfileSettingsView.test.ts app/component-lab`
