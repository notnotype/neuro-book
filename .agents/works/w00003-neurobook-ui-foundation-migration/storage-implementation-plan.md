...
- 状态：切片1、切片2、切片3的Storage上下文/grid/手势核心已收口（`da4c5aca`）；2026-09-16 继续推进切片3余项——
  ...
  **切片6 调查完成**：`view-migration-inventory.md`（提交 `1bfc2268`）登记 9 个视图族 + 外壳基线的六类证据、状态归属与依赖排序；
  完整六切片的**调查与登记**到此完成；**视图迁移已开始**：t55 `files` 视图接入提交 `aec1e0d8`（左叶文件树 + 展开项 user/local）；**收尾小切片已落地**：t56 尺寸归属（World Engine project/local + 两个窗口尺寸 user/local）提交 `4bf1562a`；下一步按清单为 Markdown Studio。证据见 [核心验证](storage-core-validation.md)。
...
- **Leader 取证（2026-09-16 续）：** 生产侧尚无任何定义注册——`server/storage/host.ts:486 registerStorageStateDefinitions` 只有测试调用方，`server/plugins/` 下没有 storage 插件，
  因此 t44 的 grid 布局定义与迁移备份边界在真实应用里都不可达。切片 4/5 必须补上服务端注册（Nitro 插件 + 单一定义清单模块），否则“真实主页面可用”无从成立。
  另记容量硬约束：单条值 ≤1 MiB、分区 ≤16 MiB（`shared/storage/contract.ts:22/28`），8 MiB 迁移原件须分块存专用分区。
