...
| Agent / Workflow | 主要链路已实现；上下文压缩已补齐有界摘要、最终请求门禁、失败 checkpoint、thrashing 保护和受验证恢复材料，真实 Project/provider/长 session 验收待做 | [Task 111](docs/tasks/111-workflow-agent-integration/README.md)、[Task 116](docs/tasks/116-agent-workflow-reliability/README.md)、[Task 147](docs/tasks/147-agent-context-compaction-reliability/README.md) |
...
- [Task 147](docs/tasks/147-agent-context-compaction-reliability/README.md) 修复了压缩摘要请求自身超窗：摘要输入/输出独立预算，所有生产 provider seam 在调用前检查最终上下文，自动摘要失败写确定性 checkpoint，重复压缩无进展时停止；本 invocation 成功文件操作可生成经 Project generation、路径、SHA-256、大小和 token budget 校验的临时恢复材料。
...
- **发布链路**：`0.9.3-canary` 的 Linux/macOS/Windows Product、Portable、容器、公开资产、manifest/checksum、GHCR 和 Windows data reuse 已通过；stable、签名安装器、updater 和最终 Desktop 方案仍未完成。
