...
| Task 145 Electron Desktop Productization | Follow-up 代码、focused/type、clean Product A/B、Portable/Depot 重复组包、仓库外 Portable 直启和当前用户安装/卸载已通过；Follow-up 后真实 machine UAC、五次冷/暖启动和 system provider 仍未重跑；原生托盘/Snap、公开签名、updater 和 macOS 实包仍未完成 | [Task 145](docs/tasks/145-electron-desktop-productization/README.md)、[ADR 0014](docs/adr/0014-electron-desktop-productization.md)、[ADR 0016](docs/adr/0016-windows-desktop-uac-broker.md)、[#87](https://github.com/notnotype/neuro-book/issues/87) |
...
- Task 145 已从最新 `origin/master` 建立生产分支并创建 Issue #87。基础 Product candidate Source revision `eac5185838479dc37ab2ff1fe6418cf46f40ba57`、image identity `sha256:e35bbfe35f04ce5a7048eb01c516b3b866fe5fa3c12786d5e707d5baed10d8bf` 已完成 Electron Manager GUI、`user|machine` 安装范围、默认关闭 auth、Provider stdin 配置、GUI 可消费的 NDJSON 完成事件和共享 Chromium Runtime；Product A/B、Portable/Depot 双组包、仓库外 Product smoke、Manager GUI/主 Electron CDP、当前用户安装/卸载和 loopback 远端 capability protocol smoke 均有证据。修复后的 machine-scope bundle 使用 Product image `sha256:06df9a0285c878a7d04fe85790731e9219ceefb3fa5e614848819404693fbba8`，真实 UAC install/repair/uninstall、HKLM、`neurobook://`、公共快捷方式和 State Root 保留策略均已验证，详见 [Task 145 final acceptance](docs/tasks/145-electron-desktop-productization/evidence/final-acceptance.json)。
- Task 145 Follow-up 已修复 machine uninstall launcher 的 Manager 相对路径、UAC repair/uninstall 缺少 `--root` 的 fail-open，并完成 `desktop/electron`、`desktop/tauri`、`desktop/shared`、`desktop/packaging` 正式命名收口。随后补上 Manager GUI launch receipt 校验和 Portable 模板时间语义；当前 focused/type/build 证据为 Manager 41 files / 295 tests、Desktop Contract 9 files / 36 tests、Electron bundle、Tauri fmt/check/release、packaging security audit 均通过。clean Product A/B 为 3241 files / 134016535 bytes，imageId 为 `sha256:1a00883a9177516d8426b9d9728d3a98b2aca0eb43fe8b31ed04daf27d734dd1`；同一 verified image 的 Electron/Tauri/Depot 重复组包逐字节一致，最终 Electron ZIP 为 389367433 bytes。仓库外 Portable headless、Manager GUI headless、最终包当前用户安装/状态/卸载和 machine 非提升 fail-closed 已通过；Follow-up 后真实 machine UAC、五次冷/暖启动和 system provider 仍未运行。
...
- **Electron beta**：Task 145 Follow-up 已达到可复核的 Windows x64 本地 candidate，Portable/用户级安装链路已重新验证；真实 machine UAC 需在本轮最终包上再跑一次，五次冷/暖启动、system provider、公开签名、后台 updater、原生托盘/Snap、macOS `.app` 和真实 Provider 成功连接仍需后续任务，因此当前仍不标记为公开 beta 发布。
