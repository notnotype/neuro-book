...
| Task 145 Electron Desktop Productization | Electron 壳门禁、Desktop Contract 11 files / 40 tests、壳门禁后的 Portable 重组包、仓库外 headless/Manager CDP 和五次冷/暖启动已通过；PR #88 必需检查全绿，但 Full tests advisory 仍有两个既有 Harness 黑盒超时。2026-08-09 由当前分支 Manager 成功完成一次 machine uninstall/UAC Host（`ok=true`，State Root 保留），并用修复后的 GUI helper 对旧 Depot 完成一次 machine install 后再卸载；该安装版启动停在 `product-spawned`，不能作为当前最终 Product image 的启动证据。旧 launcher 残留问题已由 `4a40b554` 修复，但尚未真实重跑；当前 Depot 的 machine install 仍因 UAC 被取消，未取得当前最终包的提升成功回执；原生 Snap、公开签名、updater 和 macOS 实包仍未完成 | [Task 145](docs/tasks/145-electron-desktop-productization/README.md)、[ADR 0014](docs/adr/0014-electron-desktop-productization.md)、[ADR 0016](docs/adr/0016-windows-desktop-uac-broker.md)、[#87](https://github.com/notnotype/neuro-book/issues/87) |
...
- Task 145 已从最新 `origin/master` 建立生产分支并创建 Issue #87。当前 verified Source revision 为 `8edef0f2348611abcfc312d6a2efa8603cb1797e`，Product image 为 `sha256:387f637ec10f4334d73bb749879f3d47304dffbc73ce56de37a9d37f41d78e0d`；Product A/B、Portable/Depot 双组包、仓库外 Product smoke、Manager GUI/主 Electron CDP、当前用户安装/卸载、system provider 和 loopback Supervisor 均有当前包证据。主 Electron CDP 已验证 36px 标题栏、Project、Activity Bar、右侧 Agent 面板、紧凑配置 Dialog 和 File → Quit 收口；machine 非提升路径按合同 fail-closed。2026-08-09 追加的当前分支 machine uninstall/UAC Host 已返回 `ok=true` 且保留 State Root；修复后的 GUI helper 又完成一次旧 Depot machine install 后卸载，但安装版只到 `product-spawned`。当前 Depot 的 machine install UAC 被取消，尚未取得当前最终包的提升安装成功回执，详见 [Task 145 final acceptance](docs/tasks/145-electron-desktop-productization/evidence/final-acceptance.json)。
- Task 145 Follow-up 已修复 machine uninstall launcher 的 Manager 相对路径、UAC repair/uninstall 缺少 `--root` 的 fail-open，并在 `4a40b554` 修复外置 launcher 等待 Host 最终回执后再清理自身；同时完成 `desktop/electron`、`desktop/tauri`、`desktop/shared`、`desktop/packaging` 正式命名收口。当前 focused/type/build 证据为 Manager 41 files / 299 tests、设置合同 2 files / 5 tests、Desktop Contract 11 files / 40 tests、Electron bundle、Tauri fmt/check/release、packaging security audit 均通过；Installed manifest 缺失/损坏/scope 不一致和 Electron 启动配置恢复现在均有回归门禁。clean Product A/B 为 3241 files / 134016408 bytes，imageId 为 `sha256:387f637ec10f4334d73bb749879f3d47304dffbc73ce56de37a9d37f41d78e0d`；最新 Electron Portable ZIP 为 389368123 bytes，连续两次组包逐字节一致。仓库外新 Portable headless、Manager GUI headless/CDP、当前包用户级安装/启动/状态/默认卸载、`--delete-data` 卸载、`system Bun + managed Git/rg` provider、主 Electron CDP 和 machine 非提升 fail-closed 已通过；`system Bun + system Git/rg` 会因 Bash 无版本信息 fail-closed。当前包 5×冷启动/5×暖启动也已记录；2026-08-09 当前分支 machine uninstall/UAC Host 已成功，修复后的 GUI helper 已用旧 Depot 完成一次 machine install 后卸载但安装版未完成启动，当前 Depot machine install 因 UAC 取消而未完成，Programs and Features 当前包 launcher、原生 Snap 和真实 Provider 成功连接仍未完成。
...
- **Electron beta**：Task 145 Follow-up 已达到可复核的 Windows x64 本地 candidate，Portable/用户级安装、system provider、主 Electron CDP、五次冷/暖启动和一次当前分支 machine uninstall/UAC Host 已重新验证；修复后的 GUI helper 也完成了一次旧 Depot machine install→uninstall，但安装版只到 `product-spawned`，Programs and Features launcher 的回执等待/自清理已在 `4a40b554` 修复、尚未真实重跑；当前 Depot machine install 仍因 UAC 取消而没有成功提升回执。公开签名、后台 updater、原生 Snap、macOS `.app` 和真实 Provider 成功连接仍需后续任务，因此当前仍不标记为公开 beta 发布。
