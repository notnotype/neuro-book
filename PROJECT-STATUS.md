...
| Task 145 Electron Desktop Productization | Windows 内部 beta 进行中；已迁移 Electron 启动页/Workbench，加入 Desktop Installation Manifest v2、双安装范围、Manager GUI 入口和 Provider stdin 边界；真实安装/UAC/Depot/GUI 验收未完成 | [Task 145](docs/tasks/145-electron-desktop-productization/README.md)、[ADR 0014](docs/adr/0014-electron-desktop-productization.md)、[#87](https://github.com/notnotype/neuro-book/issues/87) |
...
- Task 145 已从最新 `origin/master` 建立生产分支并创建 Issue #87。当前已把 Task 144 的 Electron 代码迁入，Manager CLI 增加 `user|machine` 安装范围、默认关闭 auth、Provider stdin 配置和 GUI 可消费的 NDJSON 完成事件；Manager GUI 与主 Electron 共用一个 Runtime，仍未完成真实 Windows UAC、安装/卸载、Depot 稳定性和 GUI 可见验收。
...
- **Electron beta**：Task 145 尚未达到可发布状态；当前用户/全局安装的注册表、快捷方式、UAC、Manager GUI 首次引导和连续组包必须在仓库外补证，未完成前不能把 Electron 称为正式安装版。
