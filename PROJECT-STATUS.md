...
| Task 145 Electron Desktop Productization | Windows x64 内部 beta candidate 已完成代码、Product/Portable 组包、Manager GUI CDP、当前用户安装/卸载和 Product 生命周期验收；全局提升安装、原生托盘/Snap、公开签名、updater 和 macOS 实包仍未完成 | [Task 145](docs/tasks/145-electron-desktop-productization/README.md)、[ADR 0014](docs/adr/0014-electron-desktop-productization.md)、[#87](https://github.com/notnotype/neuro-book/issues/87) |
...
- Task 145 已从最新 `origin/master` 建立生产分支并创建 Issue #87。当前 Product candidate Source revision `eac5185838479dc37ab2ff1fe6418cf46f40ba57`、image identity `sha256:e35bbfe35f04ce5a7048eb01c516b3b866fe5fa3c12786d5e707d5baed10d8bf` 已完成 Electron Manager GUI、`user|machine` 安装范围、默认关闭 auth、Provider stdin 配置、GUI 可消费的 NDJSON 完成事件和共享 Chromium Runtime；Product A/B、Portable/Depot 双组包、仓库外 Product smoke、Manager GUI/主 Electron CDP、当前用户安装/卸载均有证据，详见 [Task 145 final acceptance](docs/tasks/145-electron-desktop-productization/evidence/final-acceptance.json)。
