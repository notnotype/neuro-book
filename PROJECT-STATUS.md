...
- [Task 149](docs/tasks/149-repository-screenshot-runner/README.md) 已建立 `nbook.repository-research-run/v1` 研究回执、Product-first/Source Dev fallback Preview Runtime、真实 `/plot-workbench.preview` 双视口截图和 Hermes `MEDIA:` 回传 adapter。2026-08-14 在 Edge 上完成 Product-only、auto Product 优先和 Source Dev 的真实 PNG/manifest 验收；Product image `sha256:3de9f0a3cf5b57b5b29e29ec3f4df2c99f69e1ad8edef06ce71e64d204790855`，`3251 files / 136642299 bytes`。截图服务、浏览器、Product stage、端口和 owned State/Cache 均有收口证据；窄屏 manifest 仍记录 `horizontalOverflow=true`，不是被 runner 隐藏的产品事实。
- Task 149 的 Hermes 插件适配已注册通用 `repository-research` Skill；组合 smoke 输出 `tools=9 hooks=5 plugin_skill=True pi_ai_trigger=True reconnect=True slash_commands=True`。真人 QQ 客户端入站与目标客服群顺序回传仍未验证，不能用离线 smoke 或 `MEDIA:` 文件复制替代。
