...
| Task 143 Desktop Envelope | Windows-first Electron/Tauri、Manager CLI 安装合同和共享 Workbench Chrome 可见验收已通过；原生窗口完整矩阵、WebView2 分发、签名发行与最终框架选择仍未完成 | [Task 143](docs/tasks/143-desktop-envelope-installation-spike/README.md) |
...
- Task 143 已完成 Windows x64 的 Electron/Tauri Portable、ASAR、Manager CLI 用户级安装、Desktop Bridge/Supervisor、动态 loopback、认证关闭和 Tauri Job Object forced smoke；证据见 [Task 143 walkthrough](docs/tasks/143-desktop-envelope-installation-spike/README.md)。共享 Workbench Chrome、书架/Project、Inline Editor Agent 和 Agent/IDE 切换已完成 Edge headless/headed 验收；Electron 真包已覆盖 Workspace Files 与 Project Presence SSE 的 graceful shutdown。原生拖动/Snap/托盘/对话框、Agent Jobs/Session 与 WebSocket 的真包 UI、WebView2 分发、签名安装器/updater、macOS 实际包和完整 crash/disconnect 矩阵仍未完成。
...
- Task 143 的 Electron 长连接收口使用 Source `5b6a90a8` 完成新的 Product A/B、Portable P1/P2 和仓库外 CDP 验收。Product 为 3,242 文件 / 134,536,764 bytes、imageId `sha256:abeaf860cc54ef39a46861414ba31a4124ae902016842f3ba74ff64a6f3add2c`；Electron ZIP 为 389,600,709 bytes，重复组包的固定 7 个输出文件逐字节一致。真实 Workspace Files 与 Project Presence SSE 在认证 shutdown 时主动结束，Envelope 报告 `graceful`，包内进程、CDP/Product 端口和 State Root 句柄均收口；聚焦 6 文件 / 33 项、根/Runtime typecheck 和 Desktop Contract 8 文件 / 30 项通过。详见 [Electron SSE shutdown evidence](docs/tasks/143-desktop-envelope-installation-spike/evidence/electron-sse-shutdown-acceptance.json)。
...
- **发布链路**：仍需完成 Linux/macOS baseline、正式五平台 Candidate、真实 Docker/rootless Podman、最终 Portable verifier、桌面壳 crash/disconnect 与剩余长连接矩阵，以及公开 Release 证据。
