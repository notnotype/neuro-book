...
下限取 **92%**（实测通过的值），暗色下是否需要不同的数留待走查。模糊配方、色相、饱和度、亮度全部由主题自由决定。
...
1. **超出三层怎么办**——钳制在第三档、报错、还是允许更多层？Carbon 官方文档里查不到它的做法。
2. **暗色下的下限**是不是同一个数，未走查。
3. **层级推导的实现机制**：provide / inject 还是 CSS 计数器还是别的，未设计。
4. **产品其他页面是否也收敛到一档**——Lab 是仪器页，写作页、设置页需要各自走查一遍，才能知道层级轴实际会用到几档。
