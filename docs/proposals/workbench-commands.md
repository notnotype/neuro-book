...
- **命令注册与执行**：注册表和执行调度本身保留在内存；本提案不规定持久化介质或数据库变更。
- **命令面板最近使用记录（MRU）**：
  - 建议只存最近使用的命令 ID 数组，最多 30 条，由命令宿主按 [Storage 架构规范](../specs/storage/boundaries.md) 接入自己的逻辑键，建议使用 user 归属。
  - 持久化接口按 [storage.persistence](../specs/storage/persistence.md) 接入；具体键与恢复时机仍由未来命令系统设计决定，本提案不借 Storage 落地而获批。
  - 读取时校验数组与已注册命令引用，显示可回退为空；按 Storage 规范区分损坏、未知版本和读取失败，临时缺失命令或显示回退不能自动重写原记录。
