...
- **状态**：draft（方案与推荐已给出，等待开发者就「未决取舍」拍板；拍板后转 accepted 并建 Work/Task）
...
- 状态：未配置 / 已绑定 / 坏引用（modelKey 不可运行）。
- 副作用：仅影响显式传 role 的调用方。
- 失败：坏引用按模型引用问题报错；`fallback: null` 的角色不回落、继续走 profile 链。
- 验收：`roles` 段往返不丢（写 → 读 → 归一化后仍在）；role 有绑定时子代理取到该模型；未绑定时行为与今天一致。

## 决策记录

- 2026-09-10：开发者确认「UI 先行，保证 Lab 可用，后端契约缺口记录下来，正式接入时再做」；本提案即为该缺口的整理。
- 待补：开发者对「未决取舍」的结论（日期、结论）。

## 未决取舍（开工前必须定）

1. **未配置的角色怎么取模型**：推荐「继续走现有 profile 链」（不改变今天行为），另一条是「必须显式绑定，否则报配置错误」。
2. **候选链是否本期就实现运行时回退**：推荐「本期只存不消费」。
3. **role 是否允许 Project 级覆盖**：推荐「不做」（role 是设备级模型路由；Project 已有 profile 覆盖）。
4. **本地模型是否要「本地」分类与一键模板**：推荐「要，但作为 Provider 模板的产品工作，不属于本契约」。
