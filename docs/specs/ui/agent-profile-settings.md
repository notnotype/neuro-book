...
- **就地保存**：视图没有保存 / 放弃按钮；每次修改都立即把整份页面草稿交给宿主，由宿主持久化并用 `saving` / `saveError` 回报状态。
...
- Props：页面草稿 `modelValue`、上下文 `context`（作用域、继承默认 Profile、全局模型默认、全局 Profile 模型覆盖、已准备好的配置元数据与继承层、可选用途文案表）、`showNavHeading`、`loading`、`saving`、`loadError`、`saveError`、`resettingHomeKey`。
...
- 没有独立的保存事件：宿主从 `update:modelValue` 自行持久化，成功后推进自己的快照，失败则回填 `saveError` 并保留草稿；重置 Home 经确认后发出 `reset-home` 携带 profileKey。`reload` 在加载错误态发出。
...
- 运行策略字段校验失败（非法数值、超出范围的百分比等）在字段下方显示错误、展开对应分区；宿主以 `saveError` 反馈持久化失败，不静默吞掉非法数据。
...
5. Given `loading`/`load-error`/`save-error`/`saving` 场景，Then 分别呈现骨架与禁用、错误加重载、错误保草稿、保存中禁用编辑；Given `dialog-window` 场景，Then 同一受控视图在 nb-ui `DialogWindow` 内显示，关闭后可从场景内入口重新打开。
