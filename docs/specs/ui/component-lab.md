...
- 当前未完成范围：t09 `LabShell.vue <800` 仍延期；产品主题 `theme.system` clean cutover 尚未开始；nb-ui 全量 E2E 仍有 17 项既有失败；本轮治理工作树的隔离运行验收与人工视觉验收尚未获授权。
- 本轮分层输入迁移覆盖全部 132 个可挂载组件、468 个场景；登记统一使用 `{props?, model?, slots?}`，无可编辑 JSON 输入的三个组件声明 `noInput` 理由。运行时能力留在 fixture 内存中，不写入 JSON 场景；本轮静态检查与行为证据记录在治理 Work 的 t06 Task 快照，未执行的隔离浏览器验收不计入证据。
