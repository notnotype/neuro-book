...
- 这两个 ZIP 仍是未签名 Windows spike 产物，不是正式安装器、updater、卸载器或框架选型结论。真实窗口/SSE/WebSocket/Monaco/TipTap/剪贴板/拖放/文件对话框、冷启动/RSS、WebView2 分发、跨平台 runner，以及 Tauri 生产级 Owned Process/Job Object forced fallback 仍需后续授权和任务。
