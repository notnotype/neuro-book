...
- 开发者批准目标、范围和关键取舍后，主 Agent 可自主执行范围内本地可逆开发动作；远端只读访问（Issue/PR/CI/元数据）无需单独授权，按最小必要字段读取。远端Issue/Project/PR写入、push、合并、发布、部署、数据库迁移、真实Provider/Model、人工评测（Agent 浏览器自检除外）和数据删除仍需分别明确授权。advisor建议、检查通过和沉默不等于受限动作授权。
...
- 任务中出现意外失败或绕路，用户纠正做法、指出规范未被遵守或提出新规范，或者发现规范缺失或矛盾时，按 [`task-reflection`](../.agents/skills/task-reflection/SKILL.md) 在交付时分类列出回写建议，经开发者批准后再写入对应真相源。