...
- 开发者批准目标、范围和关键取舍后，主 Agent 可自主执行范围内本地可逆开发动作；远端只读访问（Issue/PR/CI/元数据）无需单独授权，按最小必要字段读取。远端Issue/Project/PR写入、push、合并、发布、部署、数据库迁移、真实Provider/Model、浏览器人工验收和数据删除仍需分别明确授权。advisor建议、检查通过和沉默不等于受限动作授权。
