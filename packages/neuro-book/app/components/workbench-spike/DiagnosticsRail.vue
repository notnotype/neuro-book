...
import type {SpikeContextValues} from "./context";
...
const emit = defineEmits<{
    (event: "toggle-context", key: keyof SpikeContextValues): void;
    (event: "move-panel"): void;
    (event: "inject", kind: "unknown-ref" | "invalid-root" | "single-leaf-root" | "version-mismatch" | "duplicate-ref" | "empty-branch"): void;
    ...
    <aside class="flex w-[360px] shrink-0 flex-col gap-[var(--space-3)] overflow-y-auto border-l border-[var(--divider)] p-[var(--space-4)]">
        <section class="flex flex-col gap-[var(--space-2)]">
            ...
            <div class="flex flex-wrap gap-[var(--space-2)]">
                ...
                <Button size="sm" variant="secondary" @click="emit('toggle-context', 'session')">会话：{{ context.session ? "开" : "关" }}</Button>
                <Button size="sm" variant="secondary" @click="emit('toggle-context', 'files')">文件上下文：{{ context.files ? "开" : "关" }}</Button>
        ...
        <section class="flex flex-col gap-[var(--space-2)]">
            ...
            <div class="flex flex-wrap gap-[var(--space-2)]">
                <Button size="sm" variant="secondary" @click="emit('move-panel')">
                    {{ panelInFullRow ? "把面板移回编辑器所在列" : "把面板移到整行（moveLeaf）" }}
                </Button>
                <Button size="sm" variant="secondary" @click="emit('inject', 'unknown-ref')">注入未知 ref</Button>
                <Button size="sm" variant="secondary" @click="emit('inject', 'invalid-root')">注入非法根节点</Button>
                <Button size="sm" variant="secondary" @click="emit('inject', 'single-leaf-root')">注入单叶根</Button>
                ...
                <Button size="sm" variant="secondary" @click="emit('collapse-left-sidebar')">收起主侧栏</Button>
                <Button size="sm" variant="secondary" @click="emit('expand-left-sidebar')">展开主侧栏</Button>
        ...
        <div class="grid grid-cols-2 gap-[var(--space-3)]">
            <section class="flex min-w-0 flex-col gap-[var(--space-1)]">
                <h2 class="text-[length:var(--text-xs)] [font-weight:var(--weight-strong)] text-[var(--text-main)]">结果（notice）</h2>
                <p v-if="notices.length === 0" class="text-[length:var(--text-2xs)] text-[var(--text-muted)]">暂无</p>
            ...
            <section class="flex min-w-0 flex-col gap-[var(--space-1)]">
                ...
                <p v-if="issues.length === 0" class="text-[length:var(--text-2xs)] text-[var(--text-muted)]">暂无</p>
