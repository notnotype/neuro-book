...
没有 props、没有 slots、没有 `expose`；`attrs` 透传到根元素。明面输出只有标题动作的两个事件（`WorkbenchViewInstances` 转发、`useWorkbenchViewActions` 收）：

```ts
type Emits = {
    /** 标题动作的运行时状态：加载中就是 busy 且禁用并给出原因。 */
    "actions-change"(states: readonly {id: string; enabled: boolean; reason?: string; busy?: boolean}[]): void;
    /** 执行句柄：把本实例的 `refresh` 交给宿主；卸载时发 `null` 交还。 */
    "action-handle-ready"(handle: {runAction(actionId: string): Promise<CommandResult<unknown>>} | null): void;
};
```

输入只有 store 与两个会话，因此调用方没有可替换的口子，也拿不到内部状态。
...
- **头部**（固定）：只有搜索框（唯一吸收横向余量的块）；一条底边线，底色取 `--bg-panel`。刷新不在这里——它是视图的**标题动作**（`SHELL_FILES_VIEW.titleActions` 的 `refresh`），按钮长在容器宿主给的那一行上。
...
搜索只会改树容器的内容，不动头部与明细面板：`390 × 844` 下头部仍是一行搜索，明细面板照常占底部一块。

## 交互

- **搜索**：纯客户端过滤（`path` / `title` / `summary` / `entryType` / `status`，大小写不敏感）；有搜索词时命中节点的祖先**强制展开**，清空搜索词回到记录的展开集。
- **刷新**（标题动作 `refresh` → 命令 `nbook.view.refresh-files`）：调 `store.loadWorkspaceTree()`；只有用户点了才拉。加载中动作 `busy` 且禁用，失败以结构化结果交回宿主展示。
...
- `WorkspaceFilePanel.test.ts`（store 与记录替身、子组件 stub）：读取中就绪前不渲染树、展开项来自记录、挂载零浏览器存储读写、选中/打开、三种明细分派、诊断条与重试/放弃；标题动作句柄走原加载入口、加载中禁用、失败结构化、内容头没有第二个刷新入口。
...
- **搜索框当前没有可访问名称**（只有 placeholder）：刷新按钮已随标题动作迁走（它的名字来自命令元数据），搜索框这一条仍是已登记的缺口。
