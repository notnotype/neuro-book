...
import AgentTextBubble from "../bubbles/text/AgentTextBubble.vue";
import AgentToolBubble from "../bubbles/tools/AgentToolBubble.vue";
...
const props = withDefaults(defineProps<{
    ...
    branchSwitcherStateByMessageId: () => ({}),
...
const emit = defineEmits<{
    ...
    (e: "empty-select-session", sessionId: number): void;
...
const getNodeKey = (node: ReturnType<typeof toChatNodes>[0]): string => {
...
<template>
    ...
    <div
        ref="scrollRef"
        class="agent-chat-flow flex flex-1 flex-col overflow-y-auto [scrollbar-gutter:stable] p-4 pb-12 bg-[var(--bg-panel)] select-text"
        @scroll="onScroll"
    >
        <!-- 顶部历史加载区 -->
        <slot name="history-loader">
            <AgentChatHistoryLoader
                :has-previous="props.historyHasPrevious"
                :loading="props.historyLoading"
                :error="props.historyError"
                @load="requestPreviousHistory"
            />
        </slot>
        ...
        <slot v-else name="empty">
            <AgentChatEmptyState
                ...
                @select-starter="emit('empty-select-starter', $event)"
                @select-session="emit('empty-select-session', $event)"
                @create-session="emit('empty-create-session')"
                @open-sessions="emit('empty-open-sessions')"
            />
        </slot>
    </div>
</template>
