...
import { resolveToolRenderConfig } from "../registry/tool-render-registry";
...
<template>
    <div v-if="renderConfig.mode !== 'hidden'" class="w-full overflow-hidden rounded-xl border border-[var(--border-color)] bg-[var(--bg-subtle)] shadow-sm">
        ...
        <!-- 展开内容区域 -->
        <div v-if="props.expanded" class="border-t border-[var(--border-color)] bg-[var(--bg-subtle)] px-3 pb-3 pt-1">
            ...
            <template v-else>
                <div class="mt-2 space-y-2 overflow-y-auto max-h-[500px] pr-1">
                    ...
                    <div>
                        <div class="mb-1 text-[10px] font-semibold uppercase tracking-[0.24em] text-[var(--text-muted)]">Arguments</div>
                        <JsonViewer v-if="parsedArgs !== null" :value="parsedArgs" :max-height="300" />
                        <div v-else class="break-all whitespace-pre-wrap rounded border border-[var(--border-color)] bg-[var(--bg-panel)] p-2 font-mono text-xs text-[var(--text-secondary)]">
                            {{ props.toolCall.argsJson ?? props.toolCall.argsText }}
                        </div>
                    </div>
                    <!-- Result：优先用 JsonViewer，fallback 到纯文本 -->
                    <div v-if="props.toolCall.result">
                        <div class="mb-1 text-[10px] font-semibold uppercase tracking-[0.24em] text-[var(--text-muted)]">Result</div>
                    ...
                    <div v-if="props.toolCall.error">
                        <div class="mb-1 text-[10px] font-semibold uppercase tracking-[0.24em] text-[var(--text-muted)]">Error</div>
