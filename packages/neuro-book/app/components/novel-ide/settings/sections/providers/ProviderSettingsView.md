...
Component Lab 中由 `ProviderSettingsViewFixture` 提供确定性场景（default / project / no-provider / disabled-models / dialog-window / saving / loading），假数据由 `fixtures/model-settings-fixture-data.ts` 构造并与设置外壳 fixture 共用；`dialog-window` 场景把本视图摆进 nb-ui `DialogWindow`，也就是产品里承载它的方式。三个对话框在 Lab 里各有自己的组件条目（`NovelIdeModelEditDialog` / `ModelDiscoveryDialog` / `ModelLibraryDialog`），不挂在本视图的场景里。
...
`NovelIdeModelSelect`（默认模型下拉）、`SavedModelsList`（已保存模型清单）与三个对话框都是 `components/` 下的既有子组件，本层只负责组合与传参：`SavedModelsList` 的五个模型行动作与两个底部入口逐一转成同名 emit，只有分组折叠留在视图内。
