 ...
 * 按整段替换，只写 `{costCurrency}` 会把 `theme` 与 `customThemes` 一起抹掉。
 */
 ...
 *
 * 旧面板在这里会给缺失的 `theme` / `customThemes` 补 `"sepia"` 与 `[]`，等于替用户选主题；
 * 新实现原样透传，缺省交给后端 `UiConfigDtoSchema` 的字段默认值（值与旧兜底一致，但责任在 schema）。
 */
