...
「桌面应用」区段的受控视图：设备说明（连接方式与版本）、界面缩放、系统托盘、关闭窗口行为。视图只消费 props 并把改动通过 `update:settings` 交回宿主，自己不探测桌面环境——`window.neuroBookDesktop` 的探测、拉取时机与失败提示都是宿主策略（Desktop Envelope 里真值在 Desktop Bridge，网页宿主下整段不渲染）。旧宿主 `NovelIdeSettingsDialog` 继续负责调 `bridge.updateSettings` 并就地覆盖设置，产品接线时再消费本视图。
...
`status` 为 `null` 时只省略「本地服务 / 远端服务 · 版本」那一行，其余照常渲染：是否渲染整个区段由宿主决定（旧宿主用 `desktopAvailable` 判断），视图不再做第二次可用性判断。缩放值不在这里夹紧——`updateSettings` 的契约由 `patchDesktopSettings` 校验，越界直接失败。
