 ...
 *   2. 栏        —— 三块浮板。左栏是冷色实心，右栏是暖色实心，
 *                   中栏**不给面**——它是一扇开向桌面的窗，所以中栏空白处看到的是第 1 层。
 *                   左栏一度是玻璃，关掉的理由见 CollapsibleSidePanel 的 --nav 那段。
 ...
 * 第 1 层：桌面。
 *
 * 这一套的用处与画布底不同：画布底是给**被测组件**当背景，桌面是三栏之间那条缝和中栏空白处
 * 看到的东西。
 *
 * 它原本还兼一件诊断的活——左栏是 26% 的玻璃时，换成棋盘格或斜纹，透了多少、模糊糊掉多少
 * 一眼就有。玻璃关掉之后这一页只剩中栏是透的，而中栏是「不给面」，压根不糊。棋盘格与斜纹
 * 眼下只剩看中栏用，保留是因为改回玻璃时立刻要用。
 *
 ...
 * 照片能看出玻璃边缘的折射位移——那需要背后有硬边内容，CSS 图案给不了；同样要等玻璃回来才用得上。
 */
