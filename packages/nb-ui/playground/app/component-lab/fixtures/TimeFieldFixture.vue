...
const designOptions = [
    {label: "方案 1: macOS 键帽时分槽 (推荐)", value: "macos"},
    ...
    <FixtureShell v-model:controls="controls" :definition="definition" :scene-id="sceneId">
        ...
        <div class="mb-6 flex flex-col gap-3">
            <div class="flex flex-wrap items-center justify-between gap-3 rounded-xl px-4 py-2.5 bg-[color-mix(in_srgb,var(--bg-panel)_75%,transparent)] backdrop-blur-xl border border-[color-mix(in_srgb,var(--border-color)_70%,transparent)] shadow-sm">
                <div class="flex flex-wrap items-center gap-2">
                    <span class="text-xs font-semibold text-[var(--text-secondary)] shrink-0">TimeField 方案:</span>
                    <SegmentedControl
                        v-model="designStyle"
                ...
                <span class="text-xs text-[var(--text-muted)]">点击各时间段体验上下键步进与焦点穿梭</span>
            </div>
            ...
            <div class="scheme-banner">
                <div class="flex items-center gap-2">
                    ...
                    <span v-if="designStyle === 'macos'" class="scheme-banner-text">
                        <strong>方案 1：macOS 键帽时分槽（推荐）</strong>——深灰 6% 实底微凹槽；时分段为超椭圆微键帽，聚焦段平滑显现 <strong>品牌蓝底色与 2.5px 柔光光晕</strong>。
                    </span>
                    ...
                    <span v-else-if="designStyle === 'outlined'" class="scheme-banner-text">
                        <strong>方案 4：精工工控独立分段方框</strong>——时、分各以 1px 细线方块独立封装，工业精密感。
                    </span>
        ...
        <div class="macos-compact-card space-y-6 max-w-md">
            <div class="flex items-center justify-between border-b border-[color-mix(in_srgb,var(--border-color)_50%,transparent)] pb-3">
                <div class="flex items-center gap-2">
                    <span class="i-lucide-timer h-4 w-4 text-[var(--accent-main)]" aria-hidden="true" />
                    <h3 class="text-sm font-semibold text-[var(--text-main)]">专注写作计时与分段时间输入</h3>
                </div>
            ...
            <div class="space-y-4">
                <div>
                    <span class="block text-xs font-semibold text-[var(--text-secondary)] mb-1.5">单时间分段精准输入:</span>
                    <TimeField
                        id="nb-lab-target"
                        v-model="timeValue"
                        :size="size"
                ...
                <div class="pt-3 border-t border-[var(--divider)]">
                    <span class="block text-xs font-semibold text-[var(--text-secondary)] mb-1.5">时间段区间输入 (TimeRangeField):</span>
                    <TimeRangeField
                        v-model="rangeValue"
                        :size="size"
                        :disabled="disabled"
            ...
            <div class="mt-4 flex items-center justify-between border-t border-[color-mix(in_srgb,var(--border-color)_40%,transparent)] pt-3 text-[11px] text-[var(--text-muted)]">
                <span>录入时间: <code class="font-mono text-[var(--accent-main)] font-bold">{{ timeValue ? String(timeValue) : 'HH:MM:SS' }}</code></span>
