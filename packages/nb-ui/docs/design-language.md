...
| | 器械（chrome） | 稿面（paper） |
| --- | --- | --- |
| 是什么 | 装工具的面：工具栏、菜单、对话框、按钮、输入框，以及只装导航的侧栏 | 装要读的东西的面：正文编辑区、章节内容，以及装文档或数据的侧栏 |
| 材质 | 半透明玻璃，磨砂 + 边缘折射 | 实心纸 |
| 色温 | 冷 | 暖 |
| 字体 | 黑体（`--font-ui`） | 宋体（`--font-display`） |
| 配色来源 | `--bg-sidebar` | `--bg-panel` |

**判据是内容不是部件。** 这张表原先按部件列举，把「侧栏」整个划给器械，于是遇到「一个装正文的侧栏」
就自相矛盾——同一条规则的后半句「玻璃不得进入长文内容面」给的是相反答案。
组件 Lab 的检视栏正是这种东西，`/lab` 里也确实按内容判成了实心，而当时没人回来改这张表。
现在改回内容判据：问「这块面装的是工具，还是要长时间读的正文与数据」，在哪一栏不重要。

还有第三种情形，它不是第三种材质：**有些区域根本不该有面。** 展台、画布要能看穿到背后的窗体底纹，
组件才有可能被放在不同的底上检视；给它上色就成了「面板里浮一块面板」。
这一档的约束是：不承载长文，也不叠任何 chrome 或 paper 的面色，浮在它上面的盒子自带面、边与抬起。
工程侧的一句话合同见 [`ui-development-spec.md`](./ui-development-spec.md) 第 2 节第 1 条。
