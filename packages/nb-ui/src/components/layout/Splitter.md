...
分隔线使用 `role="separator"`，提供与相邻面板关联的 `aria-controls`、方向、当前值、最小/最大值和像素文本。可聚焦分隔线响应主轴方向键，每步 10px，Shift 为 1px；Home / End 移到可行范围端点，Enter 切换相邻可收起面板。按键抬起时提交该次键盘会话。Escape 取消会话；失焦、浏览器夺走指针捕获、窗口失焦、外部约束/身份/方向变化或卸载也会取消而不提交。
