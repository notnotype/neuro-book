...
fn env_path(key: &str) -> Result<PathBuf, String> {
    std::env::var_os(key)
        ...
        .ok_or_else(|| format!("Tauri Desktop 缺少绝对路径环境变量：{key}"))
}

fn config() -> Result<Config, String> {
    let explicit = [
        "NBOOK_DESKTOP_DEV_PRODUCT_IMAGE_ROOT",
        "NBOOK_DESKTOP_DEV_APPLICATION_ROOT",
        "NBOOK_DESKTOP_DEV_STATE_ROOT",
        "NBOOK_DESKTOP_DEV_CACHE_ROOT",
        "NBOOK_DESKTOP_DEV_DESKTOP_ROOT",
        "NBOOK_DESKTOP_DEV_MANAGER",
        "NBOOK_DESKTOP_DEV_BUN_EXECUTABLE",
        "NBOOK_DESKTOP_DEV_PORT",
        "NBOOK_DESKTOP_DEV_REMOTE_URL",
        "NBOOK_DESKTOP_DEV_ALLOW_INSECURE_HTTP",
    ]
    .iter()
    .any(|key| std::env::var_os(key).is_some());
    if explicit {
        let port = std::env::var("NBOOK_DESKTOP_DEV_PORT")
            .map_err(|_| "Tauri Desktop 缺少 NBOOK_DESKTOP_DEV_PORT".to_string())?
            .parse::<u16>()
            .map_err(|_| "Tauri Desktop NBOOK_DESKTOP_DEV_PORT 无效".to_string())?;
        let remote_url = std::env::var("NBOOK_DESKTOP_DEV_REMOTE_URL")
            .ok()
            .filter(|value| !value.trim().is_empty())
            .map(|value| {
                validate_remote_origin(&value, env_flag("NBOOK_DESKTOP_DEV_ALLOW_INSECURE_HTTP"))
            })
            .transpose()?;
        return Ok(Config {
            image_root: env_path("NBOOK_DESKTOP_DEV_PRODUCT_IMAGE_ROOT")?,
            application_root: env_path("NBOOK_DESKTOP_DEV_APPLICATION_ROOT")?,
            state_root: env_path("NBOOK_DESKTOP_DEV_STATE_ROOT")?,
            cache_root: env_path("NBOOK_DESKTOP_DEV_CACHE_ROOT")?,
            desktop_root: env_path("NBOOK_DESKTOP_DEV_DESKTOP_ROOT")?,
            manager: env_path("NBOOK_DESKTOP_DEV_MANAGER")?,
            bun: env_path("NBOOK_DESKTOP_DEV_BUN_EXECUTABLE")?,
            port,
            remote_url,
        });
    }
    ...
    let portable_root = executable
        ...
        .ok_or_else(|| "Tauri Desktop 无法定位 portable root".to_string())?;
...
fn runtime_root(root: &Path, key: &str, fallback: PathBuf) -> Result<PathBuf, String> {
    ...
    let text = match fs::read_to_string(path) {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
            if root
                .join("desktop")
                .join("desktop-installation.json")
                .is_file()
                || is_canonical_installed_root(root)
            {
                return Err(
                    "Installed Desktop 缺少 runtime-locators.json，请通过 Manager Repair 修复。"
                        .to_string(),
                );
            }
            return Ok(fallback);
...
fn is_canonical_installed_root(root: &Path) -> bool {
    ...
    let program_files = std::env::var_os("ProgramFiles")
        .map(PathBuf::from)
        .or_else(|| {
            std::env::var_os("SystemDrive").map(|value| PathBuf::from(value).join("Program Files"))
        })
        .unwrap_or_else(|| PathBuf::from(r"C:\Program Files"));
    local_app_data
        .map(|base| base.join("Programs").join("NeuroBook"))
        .into_iter()
...
> {
    ...
    command
        ...
        .env("NBOOK_DESKTOP_DEV_BUN_EXECUTABLE", &config.bun)
...
fn main() {
    ...
    if arguments
        .iter()
        .any(|argument| argument == "--desktop-headless" || argument == "--headless")
    {
        let force_shutdown = arguments
            .iter()
            .any(|argument| argument == "--desktop-force");
    ...
    let builder = tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            let mut window_builder = WebviewWindowBuilder::new(app, "main", WebviewUrl::External(url))
                .title("NeuroBook Tauri Envelope")
