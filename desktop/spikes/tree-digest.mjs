...
const root = resolve(process.env.T140_APPLICATION_ROOT ?? "");
if (!root || root === resolve(".")) throw new Error("T140_APPLICATION_ROOT 必须是显式 Application Root");
...
const output = process.env.T140_TREE_DIGEST_OUTPUT;
if (output) await writeFile(output, `${JSON.stringify(report, null, 4)}\n`, "utf8");
const baseline = process.env.T140_TREE_DIGEST_BASELINE;
if (baseline) {
