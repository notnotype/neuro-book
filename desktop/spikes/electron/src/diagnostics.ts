...
/**
 * Electron GUI 诊断不能假定存在长期有效的父终端。
 * stdout/stderr 断开时继续运行，并把结构化事件投影到 Desktop State Root 日志。
 */
export class ElectronDiagnostics {
    ...
    private emit(
        ...
        if (this.logFile) {
            ...
            this.fileWrites = this.fileWrites
                .then(async () => {
                    await mkdir(dirname(logFile), {recursive: true});
                    await appendFile(logFile, fileLine, "utf8");
                })
                .catch(() => undefined);
        }
