...
} from "nbook/desktop/spikes/shared/src/desktop-aggregate-depot";
...
async function fixture(): Promise<{root: string; stagingRoot: string; archivePath: string; manifestPath: string}> {
    ...
    const root = await mkdtemp(join(tempRoot, "t140-aggregate-contract-"));
    ...
    const manifest = join(root, "neuro-book-desktop-spike-win-x64.manifest.json");
...
describe("Desktop aggregate depot packaging contract", () => {
    ...
    it("打包脚本消费共享合同，而不是把 Product 或 Portable 重新展开", async () => {
        const source = await readFile(join(process.cwd(), "desktop/spikes/package-portable.mjs"), "utf8");
